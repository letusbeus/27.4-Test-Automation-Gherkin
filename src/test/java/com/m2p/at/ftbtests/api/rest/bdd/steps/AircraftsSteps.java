...
public class AircraftsSteps {
    ...
    private final static String PATH_MODELNAME = PATH + "/model/{model}";
    ...
    @When("client gets details of Aircraft model={string}")
    public void getByModelName(String model) {
        var response = apiCalls.doGet(SC_OK, AircraftDto.class, PATH_MODELNAME, model);
        storage.setLastApiCallSingleItemResponse(response);
    }
    ...
    public void create(String manufacturer, String model, String numberOfSeats) throws JsonProcessingException {
        var data = CreateAircraftDto.of()
                .manufacturer(manufacturer)
                .model(model)
                .numberOfSeats(Integer.valueOf(numberOfSeats))
                .build();
    ...
    @When("client tries to create an Aircraft having manufacturer={string} and model={string} and null number of seats")
    ...
    @Then("aircraft data to be manufacturer={string} and model={string} and null number of seats")
    @Then("returned aircraft data to be manufacturer={string} and model={string} and null number of seats")
    public void returnedAircraftDataToBeManufacturerTestAndModelTestAndNullNumberOfSeats(String manufacturer, String model) {
        var lastResponse = storage.getLastApiCallSingleItemResponse();
        assertThat(lastResponse.getManufacturer())
                .as("Seems Aircraft response contained unexpected manufacturer value.")
                .isEqualTo(manufacturer);
